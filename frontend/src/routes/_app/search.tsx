...
function SearchPage() {
  ...
  const pubs = term ? PUBLICATIONS.filter((p) => p.title.toLowerCase().includes(term) || p.content_text?.toLowerCase().includes(term)) : [];
  const comms = term ? COMMUNITIES.filter((c) => c.name.toLowerCase().includes(term) || c.description.toLowerCase().includes(term)) : [];
  const users = term ? USERS.filter((u) => u.username.toLowerCase().includes(term)) : [];
...
function CommsGrid({ items }: { items: typeof COMMUNITIES }) {
  return (
...
function UsersGrid({ items }: { items: typeof USERS }) {
  return (
    <div className="grid sm:grid-cols-2 gap-3">
      {items.map((u) => (
        <Link key={u.id} to="/users/$id" params={{ id: u.id }} className="rounded-xl border border-border bg-card p-3 hover:border-primary/40">
          <div className="flex items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs text-muted-foreground">{u.career?.name}</div>
            </div>
          </div>
        </Link>
      ))}
